...
class ThumbnailGenerator:
    ...
    def generate_thumbnail(self, story_text, bottom_text, bg_color, text_font_size, text_font_family,
                          ...
                          headshot_x_offset=0, headshot_y_offset=0, headshot_width=340, headshot_height=500):
        """Generate thumbnail with all settings including custom headshot positioning"""
        ...
        # Calculate base headshot position
        if headshot_position == "Left":
            base_headshot_x = 40
            text_area_x = base_headshot_x + headshot_width + 60
            text_area_width = width - text_area_x - 40
        else:  # Right
            text_area_x = 40
            text_area_width = width - headshot_width - 140
            base_headshot_x = width - headshot_width - 40

        base_headshot_y = 80

        # Apply custom offsets from sliders
        final_headshot_x = base_headshot_x + headshot_x_offset
        final_headshot_y = base_headshot_y + headshot_y_offset

        # Ensure headshot stays within bounds
        final_headshot_x = max(0, min(final_headshot_x, width - headshot_width))
        final_headshot_y = max(0, min(final_headshot_y, height - headshot_height))
        ...
        img.paste(headshot_resized, (final_headshot_x, final_headshot_y))
...
class ThumbnailGeneratorApp:
    ...
    def run(self):
        ...
        # NEW: Headshot sizing and positioning controls
        st.markdown("### 📐 Headshot Size & Position Controls")

        col_size1, col_size2 = st.columns(2)
        with col_size1:
            headshot_width = st.slider(
                "Headshot Width (px)",
                min_value=200,
                max_value=600,
                value=500,
                step=10,
                key="tg_headshot_width"
            )
        with col_size2:
            headshot_height = st.slider(
                "Headshot Height (px)",
                min_value=300,
                max_value=700,
                value=630,
                step=10,
                key="tg_headshot_height"
            )

        col_pos1, col_pos2 = st.columns(2)
        with col_pos1:
            headshot_x_offset = st.slider(
        ...
        st.info(f"📍 Current offset: X={headshot_x_offset}px, Y={headshot_y_offset}px | Size: {headshot_width}x{headshot_height}px")
        ...
        if "Random" in headshot_mode:
            ...
            if uploaded_files:
                ...
                position = st.selectbox("Base Headshot Position:", ["Right", "Left"], key="tg_random_pos")
            ...
            for idx in st.session_state.tg_selected_stories:
                ...
                with st.expander(f"Story {story['story_number']}", expanded=False):
                    ...
                    with col2:
                        position = st.selectbox("Base Position:", ["Right", "Left"], key=f"tg_ind_pos_{idx}")
        ...
        if st.session_state.tg_selected_stories and len(headshots_data) > 0:
                ...
                if first_idx not in headshots_data:
                    ...
                    with st.spinner("Generating..."):
                        ...
                        thumbnail = self.generator.generate_thumbnail(
                            ...
                            headshot_x_offset=headshot_x_offset,
                            headshot_y_offset=headshot_y_offset,
            ...
            if st.session_state.tg_preview_image:
                st.image(st.session_state.tg_preview_image, width='stretch')
